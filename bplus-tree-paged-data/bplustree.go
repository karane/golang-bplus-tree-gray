	...
	for {
		...
		if node.isLeaf {
			...
			if i < len(node.keys) && node.keys[i] == key {
				v, err := t.store.ReadRecord(node.ptrs[i])
				if err != nil {
					return "", false, err
				}
				return v, true, nil
...
func (t *BPlusTree) insert(nodeID int32, key int64, value string) (int64, int32, error) {
	...
	if node.isLeaf {
		i := sort.Search(len(node.keys), func(i int) bool { return node.keys[i] >= key })
		if i < len(node.keys) && node.keys[i] == key {
			// Overwrite: append new record; old record is orphaned (append-only data file).
			ptr, err := t.store.AppendRecord(value)
			if err != nil {
				return 0, noPage, err
			}
			node.ptrs[i] = ptr
		...
		node.keys[i] = key
		node.ptrs = append(node.ptrs, RecordPtr{})
...
func (t *BPlusTree) splitLeaf(node *Node) (int64, int32, error) {
	...
	right.ptrs = append([]RecordPtr{}, node.ptrs[mid:]...)
